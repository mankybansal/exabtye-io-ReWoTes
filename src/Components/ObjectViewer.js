...
class ObjectViewer extends Component {
  ...
  createGroup() {
    ...
    for (let i = 0; i < this.viewerFile.atomCount; i++) {
      ...
      cubes[i].position.set(
        this.viewerFile.atoms[i].x,
        this.viewerFile.atoms[i].y,
        this.viewerFile.atoms[i].z
      );
      group.add(cubes[i]);
    }
  ...
  processCode(code) {
    ...
    this.scene
      .getObjectByName("parentGroup")
      .remove(this.scene.getObjectByName("atomGroup"));
  ...
  animate() {
    ...
    if (this.props.viewerRotate) this.autoRotate();
